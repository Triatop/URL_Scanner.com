...
def createuser():
    ...
    db_obj.createUser(new_username, new_password,
                      new_fullname, new_token, new_adminPriv)
...
def history():
    ...
    for index, value in enumerate(history):
        # convert keys to integers as to jsaonb converts keys to strings.
        a_dict = {int(k): v for k, v in value[3].items()}
        rmaker.createReport(a_dict, value[4], value[5], value[6])
        histDict[index] = {'url': u_ctrl.decryptUrl(value[0]), 'date': str(
            value[1]), 'safe': value[2], 'report': rmaker.getReport()}
