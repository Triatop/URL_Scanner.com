...
class DBController:
    ...
    def getPrevScans(self):
        try:
            ...
            self.cur.execute(query)
            self.conn.commit()
            url_attr_lst = list({'url': r[0], 's_value': r[1], 'attributes': r[2], 'site_age': r[3],
                                'mal_links': r[4], 'char_swap_url': r[5]} for r in self.cur.fetchall())
            for index, val in enumerate(url_attr_lst):
    ...
    def loginUser(self, username, password):
        try:
            query = '''select en_pw, salt from users where username = '{}' '''.format(
                username)
            self.cur.execute(query)
            self.conn.commit()
    ...
    def getUserToken(self, username):
        try:
            query = '''select access_token from users where username = '{}' '''.format(
                username)
            self.cur.execute(query)
            self.conn.commit()
    ...
    def authenticateAdmin(self, username):
        try:
            query = '''select rights_name from users inner join rights on users.rights_id = rights.rights_id and users.username = '{}' '''.format(
                username)
            self.cur.execute(query)
            self.conn.commit()
    ...
    def createUser(self, username, password, fullname, a_token, admin_priv):
        ...
        try:
            if (admin_priv):
                self.user_value = 3681
            query = '''insert into users(rights_id, username, en_pw, salt, fname, lname, rg_date, access_token) values('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')'''.format(
                self.user_value, username, en_pw, salt, fname, lname, rg_date, a_token)
            self.cur.execute(query)
            self.conn.commit()
        except (Exception, psycopg2.Error) as error:
            self.conn.rollback()
    ...
    def validateUser(self, uname, token):
        try:
            query = '''select count(*) from users where username = '{}' and access_token = '{}' '''.format(
                uname, token)
            self.cur.execute(query)
            self.conn.commit()
            return self.cur.fetchone()[0] == 1
    ...
    def insertScan(self, username, url, s_value, a_dict, site_age, mal_links, char_swap_url):
        ...
        try:
            ...
            self.cur.execute(query, (user_id, url, s_value, json.dumps(
                a_dict), site_age, mal_links, char_swap_url))
            self.conn.commit()
        except (Exception, psycopg2.Error) as error:
            self.conn.rollback()
